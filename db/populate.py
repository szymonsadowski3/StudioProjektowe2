...
sections = [Section(name=faker.color_name(), section_logo=None) for _ in range(GENERATION_CFG["sections"])]
...
posts = [
    Post(title=faker.catch_phrase(), post_content=get_text(), parentThread=random.choice(threads)) for _ in range(GENERATION_CFG["posts"])
]
